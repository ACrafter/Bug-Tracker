...
export class ProjectStore {
  ...
  async create(projectInfo: Project): Promise<Project> {
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "INSERT INTO projects (title, project_lang, project_desc ,department, rating, last_mod_by) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *"; // Defining the SQL query
      const result = await connection.query(sql, [
        projectInfo.title,
        projectInfo.project_lang,
        projectInfo.project_desc,
        projectInfo.department,
        projectInfo.rating,
        projectInfo.last_mod_by,
      ]); // Running the SQL query on the DB & storing the result
      connection.release(); // Closing the connection
      return result.rows[0]; // Returning the result
    } catch (err) {
      throw new Error(
        `Couldn't create project with title=${projectInfo.title} => ${err}`
      );
    }
  }
  ...
  async update(
    ...
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "UPDATE projects SET " + modify + "=$2 WHERE id=$1 RETURNING *"; // Defining the SQL query
