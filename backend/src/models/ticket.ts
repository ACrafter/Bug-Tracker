...
export class TicketStore {
  ...
  async create(ticketInfo: Ticket): Promise<Ticket> {
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "INSERT INTO tickets (title, ticket_type, ticket_Description ,projectid) VALUES ($1, $2, $3, $4) RETURNING *"; // Defining the SQL query
      const result = await connection.query(sql, [
        ticketInfo.title,
        ticketInfo.ticket_type,
        ticketInfo.ticket_description,
      ...
      throw new Error(
        `Couldn't create ticket with title=${ticketInfo.title} => ${err}`
      );
    }
  }
  ...
  async update(
    ...
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "UPDATE tickets SET " + modify + "=$2 WHERE id=$1 RETURNING *"; // Defining the SQL query
